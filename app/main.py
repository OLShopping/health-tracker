...
def init_db():
    with get_db() as conn:
        ...
        CREATE TABLE IF NOT EXISTS bowel_records (
            ...
            date        TEXT NOT NULL,
            stool_type  INTEGER,   -- 大便形态 1-7（布里斯托分类）
            note        TEXT
        );
        ...
        """)
        # ── 数据库迁移：为旧库补充新字段 ──
        existing_cols = [row[1] for row in conn.execute("PRAGMA table_info(bowel_records)").fetchall()]
        if "stool_type" not in existing_cols:
            conn.execute("ALTER TABLE bowel_records ADD COLUMN stool_type INTEGER")
...
def delete_medicine(med_id):
    with get_db() as conn:
        row = conn.execute("SELECT photo FROM medicines WHERE id=?", (med_id,)).fetchone()
    ...
    if old_photo:
        ...
        try:
            if os.path.isfile(old_path):
                os.remove(old_path)
                print(f"[Delete] 已清理药品照片: {old_photo}")
        except OSError as e:
            print(f"[Delete] 清理照片失败（已忽略）: {e}")
...
def upload_medicine_photo(med_id):
    ...
    with get_db() as conn:
        # 查出旧照片文件名，保存新照片后删除旧文件
        row = conn.execute("SELECT photo FROM medicines WHERE id=?", (med_id,)).fetchone()
        old_photo = row["photo"] if row else None
        conn.execute("UPDATE medicines SET photo=? WHERE id=?", (filename, med_id))

    # 删除旧文件（数据库已更新后再删，避免事务回滚时文件已丢失）
    if old_photo and old_photo != filename:
        old_path = os.path.join(UPLOAD_FOLDER, old_photo)
        try:
            if os.path.isfile(old_path):
                os.remove(old_path)
                print(f"[Upload] 已删除旧照片: {old_photo}")
        except OSError as e:
            print(f"[Upload] 删除旧照片失败（已忽略）: {e}")
...
def add_bowel():
    ...
    stool_type = data.get("stool_type")   # 布里斯托分类 1-7，可为 None
    with get_db() as conn:
        conn.execute(
            "INSERT INTO bowel_records (recorded_at, date, stool_type, note) VALUES (?,?,?,?)",
            (recorded_at, recorded_at[:10], stool_type, data.get("note", ""))
        )
    return jsonify({"message": "记录成功"}), 201
